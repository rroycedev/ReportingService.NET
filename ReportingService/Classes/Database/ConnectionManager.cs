...
using ReportingService.Classes.Exceptions;
...
namespace ReportingService.Classes.Database
{
    public class ConnectionManager
    {
        ...
        {
            ConnectionManager.connectionMutex.WaitOne();
        ...
        {
  //          Console.WriteLine(String.Format("ConnectionManager: Waiting for semaphore to release connection {0}", conn.PoolId));

            connectionMutex.WaitOne();

    //        Console.WriteLine(String.Format("ConnectionManager: Releasing connection {0}", conn.PoolId));

            this.connectionPool[conn.PoolId].Release();

      //      Console.WriteLine(String.Format("ConnectionManager: Releasing semaphore for connection {0}", conn.PoolId));
